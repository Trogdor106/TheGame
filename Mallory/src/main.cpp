...
{	
	{
		...
		InstrumentationPreLayer* profilingLayer = new InstrumentationPreLayer();
		InstrumentationPostLayer* postProfiler = new InstrumentationPostLayer();
		profilingLayer->PostLayer = postProfiler;
		// Set up our layers
		app->AddLayer(profilingLayer); // Set up our profiling pre-layer as the first layer
		...
		app->AddLayer(profilingLayer->PostLayer); // Set up profiling post-layer as the last layer
