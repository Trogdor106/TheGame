...
{
	ProfileTimer timer = ProfileTimer("LightingPreRender");
	if (isProcessingShadows) {
		...
		if (view.size() > 0) {
			...
			ecs.view<ShadowLight>().each([&](auto entity, ShadowLight& light) {
				ProfileTimer timer = ProfileTimer("Shadow Render");
				...
				ProfileTimer sceneTimer = ProfileTimer("Scene");
...
void LightingLayer::PostProcessLights() { 
	...
	if (isLightOn != true) {
		myFinalComposite->SetUniform("a_Exposure", 0.0f);
	}
	else {
