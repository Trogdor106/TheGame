...
void HitBoxes::saveHitBoxes(HitBox boxToSave, int floor)
{
	switch (floor) {
	case 0:
		if (hitBoxHolderFirst[0].ID == -1) {
			hitBoxHolderFirst[0] = boxToSave;
			OGHitBoxHolderFirst[0] = boxToSave;
		}
		else {
		...
		break;
	case 1:
		if (hitBoxHolderSecond[0].ID == -1) {
			hitBoxHolderSecond[0] = boxToSave;
			OGHitBoxHolderSecond[0] = boxToSave;
		}
		else {
		...
		break;
	case 2:
		if (hitBoxHolderThird[0].ID == -1) {
			hitBoxHolderThird[0] = boxToSave;
			OGHitBoxHolderThird[0] = boxToSave;
		}
		else {
