...
Meshmini::Meshmini(Vertexmini* vertices, size_t numVerts, uint32_t* indices, size_t numIndices) {
	...
	glBufferData(GL_ARRAY_BUFFER, numVerts * sizeof(Vertexmini), vertices, GL_STATIC_DRAW);
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, false, sizeof(Vertexmini), &(vert->Position));
	...
	glVertexAttribPointer(1, 4, GL_FLOAT, false, sizeof(Vertexmini), &(vert->Color));
