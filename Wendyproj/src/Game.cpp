...
void Game::Update(float deltaTime) {
	...
	Camera::Sptr newCamera;
	newCamera->SetPosition(glm::vec3(0.0f, 0.0f, 0.0f));
	//newCamera->Rotate(glm::vec3(0, rotation.y, 0));
	//newCamera->Rotate(glm::vec3(rotation.x, 0, 0));
	
//	glm::mat4 newView = myCamera->GetView();

	//myCamera->SetView();

//	glm::vec3 globalPos;
//	globalPos = (myCamera->GetPosition());


	// Rotate and move our camera based on input
	
	myCamera->Rotate(glm::vec3(rotation.x, rotation.y, 0));
