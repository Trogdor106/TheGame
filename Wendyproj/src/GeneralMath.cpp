...
void deCasteJau::calculatedeCasteJau() {
	for (int j = 0; j < deCastleObjectList.size(); j++) {
		...
		for (int i = 0; i < deCastleObjectList[j].originalModel1.size(); i++) {
			...
			MorphObject::lerp(deCastleObjectList[j].originalModel1[i].Position, deCastleObjectList[j].originalModel2[i].Position, tempVertices, tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(deCastleObjectList[j].originalModel2[i].Position, deCastleObjectList[j].targetModel1[i].Position, tempVertices2,	tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(deCastleObjectList[j].targetModel1[i].Position, deCastleObjectList[j].targetModel2[i].Position, tempVertices3,	tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(tempVertices, tempVertices2, tempVertices4,																		tempAmountOfTimes, 10, j, currentInterval[j]);															
			MorphObject::lerp(tempVertices2, tempVertices3, tempVertices5,																		tempAmountOfTimes, 10, j, currentInterval[j]);															
			MorphObject::lerp(tempVertices4, tempVertices5, tempVertices6,																		tempAmountOfTimes, 10, j, currentInterval[j]);															
																																										
			MorphObject::lerp(deCastleObjectList[j].originalModel1[i].Normal, deCastleObjectList[j].originalModel2[i].Normal, tempNeorm,		tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(deCastleObjectList[j].originalModel2[i].Normal, deCastleObjectList[j].targetModel1[i].Normal, tempNeorm2,			tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(deCastleObjectList[j].targetModel1[i].Normal, deCastleObjectList[j].targetModel2[i].Normal, tempNeorm3,			tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(tempNeorm, tempNeorm2, tempNeorm4,																				tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(tempNeorm2, tempNeorm3, tempNeorm5,																				tempAmountOfTimes, 10, j, currentInterval[j]);
			MorphObject::lerp(tempNeorm4, tempNeorm5, tempNeorm6,																				tempAmountOfTimes, 10, j, currentInterval[j]);
