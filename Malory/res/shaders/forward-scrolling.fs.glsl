...
uniform vec2  a_ScrollDir;

uniform float a_EmissiveStrength;

void main() {
	vec2 scroll = a_ScrollDir * a_Time;
	// Write the output
	outAlbedo = vec4(texture(s_Albedo, inUV + scroll).rgb * inColor.rgb, inColor.a);

	// Output the material's emissive light
	outEmissive = texture(s_Emissive, inUV + scroll).rgb * a_EmissiveStrength;
