...
ProfilingSession* ProfilingSession::__Session = nullptr;

void ProfilingSession::Start(const std::string& outPath)
{
...
void ProfilingSession::WriteResult(const ProfilingResult& result)
{
	if (__Session != nullptr) {
...
ProfilingSession::ProfilingSession(const std::string& outPath) {
	myOutStream.open(outPath);
	_WriteHeader();
	myProfileCount = 0l;
}

void ProfilingSession::_WriteHeader()
{
	myOutStream << R"L({"otherData": {}, "traceEvents": [)L";
	myOutStream.flush();
}

void ProfilingSession::_WriteFooter() {
	myOutStream << R"L(]})L";
	myOutStream.flush();
}

void ProfilingSession::_WriteResult(const ProfilingResult& result)
{
