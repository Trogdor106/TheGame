...
void PostLayer::Initialize() {
	...
	//if (true) {
	//	// Our color correction pass will use a 3D lut to correct our color
	//	auto motionBlur = __CreatePass("shaders/post/color-correct.fs.glsl");
	//	motionBlur->Inputs.push_back({ nullptr, RenderTargetAttachment::Color0, false, Texture3D::LoadFrom3DFile("sample.cube") }); // Jank in the LUT!
	//	// Add the pass to the post processing stack
	//	myPasses.push_back(motionBlur);
	//
	//	// We will toggle motion blur on and off with one key
